...
class DivModelEval():
    ...
    def scaling_img(self, scale):
        """"
        同比例缩放
        """
        self.img_size = [int(self.img_size[0] * scale), int(self.img_size[1] * scale)]
        self.center_points = [self.center_points[0] * scale, self.center_points[1] * scale]
        self.r_max = get_rmax(self.center_points, self.img_size[0], self.img_size[1])
        self.recompute_k()
